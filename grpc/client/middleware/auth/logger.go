...
func (l *Logger) MethodNotSupported(method string) {
	l.logger.LogMessage(gologger.NewLogMessage("Method not supported", gologgerstatus.StatusWarning, nil, method))
}
...
func (l *Logger) MissingGRPCMethod(fullPath string) {
	l.logger.LogMessage(gologger.NewLogMessage("Missing gRPC method", gologgerstatus.StatusWarning, nil, fullPath))
}

// MissingGRPCInterceptions logs a MissingGRPCInterceptionsError
func (l *Logger) MissingGRPCInterceptions() {
	l.logger.LogError(gologger.NewLogError("Missing gRPC interceptions", nil, NilGRPCInterceptionsError))
}
